...
if __name__ == '__main__':
	...
	root = '/home/bailiang/codes/guanbiliang/data'
	...
	for type, id in category_ids.items():
		...
		for phrase in ['train', 'test']:
			...
			phrase_path = open(os.path.join(root,type, phrase + '.txt'))
			...
			for file in files:
				# load mesh
				mesh = pymesh.load_mesh(os.path.join(root, type, phrase,file[:-1] + '.off'))
				face_lab = np.loadtxt(open(os.path.join(root, type, phrase,file[:-1] + '.txt'), 'r'), dtype='float64')
				# get elements
